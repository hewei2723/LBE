...
def list_books():
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'book.csv'), 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            print(row)
...
def add_book():
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'books.csv'), 'a', newline='') as f:
        writer = csv.writer(f)
...
def delete_book():
    ...
    lines = []
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'books.csv'), 'r') as f:
        reader = csv.reader(f)
        lines = [row for row in reader if row[0] != book_name]

    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'books.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
